...
def deduplicate_legend(handles, labels):
    ...
    new_labels, new_handles = (
        list(t) for t in zip(*sorted(zip(new_labels, new_handles)))
    )
...
def group_hbar_plots(ax, group_size, inner_sep=None):
    ...
    for column, handle in enumerate(handles):
        for row, patch in enumerate(handle.patches):
            ...
            patch.set_y(row_within_group * (bar_height + inner_sep) + group_offset)
...
def compare_scenarios(result_files, output_filename):
    ...
    scenario_names = [
        os.path.basename(rf)  # drop folder names, keep filename
    ...
    for rf in result_files:  ##ToDO ab hier code aus "debugfile.py"
        with pd.ExcelFile(rf) as xls:
            ...
            esum.set_index(
                ["Unnamed: 0", "Unnamed: 1"], inplace=True
            )  ##ToDO personal fix
            ...
            def normalize_column_name(col):  ##ToDO created function
                if isinstance(col, str):
                    ...
                    return ".".join(
                        col[1:]
                    )  # z.B. ('2024', 'North', 'Elec') -> 'North.Elec'
                return col
            ...
            for com in coms:  ##ToDo manually changed sum each commodity
                com_sum = pd.DataFrame(
                    esum.loc[:, sit_com.str.contains(com)].sum(axis=1), columns=[com]
                )
    ...
    bp0 = spent.plot(ax=ax0, kind="barh", stacked=True, color=spent_colors, linewidth=0)
    if not earnt.empty:
        earnt_colors = [urbs.to_color(ct) for ct in earnt.columns]
        bp0a = earnt.plot(
            ax=ax0, kind="barh", stacked=True, color=earnt_colors, linewidth=0
        )

    ax1 = plt.subplot(gs[1])
    esums_colors = [urbs.to_color(commodity) for commodity in esums.columns]
    bp1 = esums.plot(
        ax=ax1, kind="barh", stacked=True, color=esums_colors, linewidth=0, width=0.5
    )
    ...
    for ax in [ax0, ax1]:
        ...
        # legend
        lg = ax.legend(
            frameon=False, loc="upper center", ncol=4, bbox_to_anchor=(0.5, 1.11)
        )
        plt.setp(lg.get_patches(), edgecolor=urbs.to_color("Decoration"), linewidth=0)
    ...
    for ext in ["png", "pdf"]:
        fig.savefig("{}.{}".format(output_filename, ext), bbox_inches="tight")
