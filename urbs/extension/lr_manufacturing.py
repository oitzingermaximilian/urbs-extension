...
class AbstractConstraint(ABC):
    @abstractmethod
    def apply_rule(self, m, *args, **kwargs):
        pass


class costsavings_rule_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        pricereduction_value_pri = sum(
            m.P_pri[n, tech, location] * m.BD_pri[stf, location, tech, n]
            for n in m.nsteps_pri
        )
        print(
            f"Calculated pricereduction for {stf}, {location}, {tech}: {pricereduction_value_pri}"
        )

        return m.pricereduction_pri[stf, location, tech] == pricereduction_value_pri


class BD_limitation_rule_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        bd_sum_value_pri = sum(m.BD_pri[stf, location, tech, n] for n in m.nsteps_pri)
        print(
            f"BD_limitation_rule for stf={stf}, Location={location}, Tech={tech}: Sum of BD is {bd_sum_value_pri}"
        )

        return bd_sum_value_pri <= 1


class relation_pnew_to_pprior_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        if stf == m.y0:
            ...
            print(
                f"Debug: relation_pnew_to_pprior_sec for stf={stf}: pricereduction[{stf}] = {m.pricereduction_pri[stf, location, tech]}, pricereduction[{stf - 1}] = {m.pricereduction_sec[stf - 1, location, tech]}"
            )
            return (
                m.pricereduction_pri[stf, location, tech]
                >= m.pricereduction_pri[stf - 1, location, tech]
            )


class q_perstep_rule_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        for year in m.stf:
            if year <= stf:  # Accumulate up to the current year (stf)
                try:
                    lhs_cumulative_sum_sec += m.capacity_solar_euprimary[
                        year, location, tech
                    ]
                except KeyError:
                    print(
        ...
        rhs_value_sec = sum(
            m.BD_pri[stf, location, tech, n] * m.capacityperstep_pri[n, location, tech]
            for n in m.nsteps_pri
        )
...
class upper_bound_z_eq_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        """
        nsteps_pri = kwargs.get("nsteps_pri")
        if nsteps_pri is None:
            raise ValueError("nsteps_pri is required for this constraint")

        lhs_value = (
            m.BD_pri[stf, location, tech, nsteps_pri]
            * m.capacity_solar_euprimary[stf, location, tech]
        )
        rhs_value = m.gamma_pri * m.BD_pri[stf, location, tech, nsteps_pri]

        # Debug: Print the left-hand side and right-hand side for upper bound comparison
        print(
            f"Debug: upper_bound_z_eq_sec for stf={stf}, nsteps_pri={nsteps_pri}: LHS = {lhs_value}, RHS = {rhs_value}"
        )

        return lhs_value <= rhs_value


class upper_bound_z_q1_eq_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        rhs_value = m.capacity_solar_euprimary[stf, location, tech]

        # Debug: Print the left-hand side and right-hand side for upper bound comparison
        print(
            f"Debug: upper_bound_z_q1_eq_sec for stf={stf}, nsteps_pri={nsteps_pri}: LHS = {lhs_value}, RHS = {rhs_value}"
        )

        return lhs_value <= rhs_value


class lower_bound_z_eq_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        rhs_value = (
            m.capacity_solar_euprimary[stf, location, tech]
            - (1 - m.BD_pri[stf, location, tech, nsteps_pri]) * m.gamma_pri
        )

        # Debug: Print the left-hand side and right-hand side for lower bound comparison
        print(
            f"Debug: lower_bound_z_eq_sec for stf={stf}, nsteps_pri={nsteps_pri}: LHS = {lhs_value}, RHS = {rhs_value}"
        )

        return lhs_value >= rhs_value


class non_negativity_z_eq_pri(AbstractConstraint):
    def apply_rule(self, m, stf, location, tech, **kwargs):
        """
        ...
        print(
            f"Debug: non_negativity_z_eq_sec for stf={stf}, nsteps_pri={nsteps_pri}: LHS = {lhs_value}"
        )

        return lhs_value >= 0


def apply_m_constraints(m):
    constraints = [
        costsavings_rule_pri(),
        BD_limitation_rule_pri(),
        relation_pnew_to_pprior_pri(),
        q_perstep_rule_pri(),
        upper_bound_z_eq_pri(),
        upper_bound_z_q1_eq_pri(),
        lower_bound_z_eq_pri(),
        non_negativity_z_eq_pri(),
    ]
